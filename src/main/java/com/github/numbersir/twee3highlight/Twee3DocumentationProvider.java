...
final class Twee3DocumentationProvider extends AbstractDocumentationProvider {
    /**
     * Extracts the key, value, file and documentation comment of a Twee3 key/value entry and returns
     * a formatted representation of the information.
     */
    @Override
    public @Nullable String generateDoc(PsiElement element, @Nullable PsiElement originalElement) {
        if (element instanceof Twee3Property) {
            final String key = ((Twee3Property) element).getKey();
            final String value = ((Twee3Property) element).getValue();
            final String file = SymbolPresentationUtil.getFilePathPresentation(element.getContainingFile());
            final String docComment = Twee3Util.findDocumentationComment((Twee3Property) element);
    ...
    /**
     * Provides documentation when a Twee3 Language element is hovered with the mouse.
     */
    @Override
    ...
    /**
     * Provides the information in which file the Twee3 language key/value is defined.
     */
    @Override
    ...
    public @Nullable PsiElement getCustomDocumentationElement(@NotNull Editor editor, @NotNull PsiFile file, @Nullable PsiElement context, int targetOffset) {
        if (context != null) {
            // In this part the Twee3Property element
            ...
            if (context instanceof PsiJavaToken && ((PsiJavaToken) context).getTokenType().equals(JavaTokenType.STRING_LITERAL)) {
                ...
                for (PsiReference ref : references) {
                    if (ref instanceof Twee3Reference) {
                        PsiElement property = ref.resolve();
                        if (property instanceof Twee3Property) {
                            return property;
                        }
                    }
                }
            }
            // In this part the Twee3Property element is extracted
            // when inside of a .twee file
            else if (context.getLanguage() == Twee3Language.INSTANCE) {
                PsiElement property = PsiTreeUtil.getParentOfType(context, Twee3Property.class);
                if (property != null) {
                    return property;
                }
            }
